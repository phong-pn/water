...
class DayLogDrinkAdapter(
    ...
    inner class LogDrinkViewHolder(itemView: View, val onLogSelect: (LogDrink) -> Unit) :
        RecyclerView.ViewHolder(itemView) {
        fun bindData(logHeader: LogDrinkHeader) {
            val log = logHeader.log
            itemView.apply {
                if (!logHeader.isHeader) header.visibility = View.GONE
                else header_day.text =
                    if (DateUtils.isToday(log.cal.timeInMillis)) context.getString(R.string.today) else formatDate(
                        log.cal
                    )
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): LogDrinkViewHolder {
        ...
        return LogDrinkViewHolder(
            inflater.inflate(R.layout.log_drink_rv_item, parent, false),
            onLogSelect
        )
    }

    override fun onBindViewHolder(holder: LogDrinkViewHolder, position: Int) =
        holder.bindData(listLogDrinkHeader[position])
    ...
    fun update(new: List<List<LogDrink>>) {
        ...
        val diffCallBack =
            object : DiffCallBack<LogDrinkHeader>(listLogDrinkHeader, newListLogDrinkHeader) {
