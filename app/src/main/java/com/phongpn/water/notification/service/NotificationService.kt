...
class NotificationService : Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        alarmSchedule = AlarmSchedule.get()
        val amount =
            LogDrinkRepo.totalAmountByIdDate(LogDrink(0, "", Calendar.getInstance()).idDate)
        val goalToday = SharePrefUtil.goal
        if (amount < goalToday && haveScheduleToday()) {
            intent?.let {
                ...
                alarmSchedule.apply {
                    ...
                    when (sound) {
                        ...
                        AlarmSchedule.SOUND_WATER -> Uri.parse(
                            "android.resource://" +
                    ...
                    notificationBuilder.apply {
                        setContentText(getString(R.string.you_need_to_get) + ": ${goalToday - amount} ml}")
                    }
